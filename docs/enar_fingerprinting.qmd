...
#| cache: true
#| include: false 
xdf = read_csv(here::here("data", "iu_subj2.csv"))

xdf = 
  xdf %>% 
  filter(activity == 4) %>% 
  mutate(signal = sqrt(lw_x^2 + lw_y^2 + lw_z^2)) %>% 
  slice(1:(10*100)) %>% 
  mutate(t = (row_number() -1) / 100) %>% 
  select(signal, t)
...
**Join the Mobile and Wearable Data Science ASA Interest Group!** 
[wearableds.github.io/involvement](wearableds.github.io/involvement)
...
![](figs/wearable_DS_QR.png)

:::: 

:::: {.column}

![](figs/interest_group_logo.jpg)
...
p3v2 = 
  ...
  geom_line(linewidth = 0.9, color = "#000000")+
  theme(panel.grid.minor = element_blank()) +
  labs(x = "Time (s)", y = "Acceleration (g = 9.81m/s²)") +
  scale_x_continuous(breaks = seq(0,10,1)) + 
  scale_y_continuous(limits = c(0.5, 1.75)) +
  annotate(geom = "label", x = 5, y = 1.5, label = "??", size = 8, color = "#000000")
...
p3v2_annotated = p3v2 + plot_annotation(
  caption = "80 observations per second\n× 60 seconds/minute\n× 1440 minutes/day\n× 7 days\n× 15,000 individuals\n=726 billion data points (10 Tb)",
  theme = theme(
    plot.caption = element_text(
      hjust = 0.5,            # center the text
      vjust = 1,              # bring it up a bit
...
### The method works really well when we know people are walking
...
```{r accel plot nhanes 2}

nh_img_plot + 
  wrap_elements(p3v2_annotated) + 
  plot_layout(widths = c(1, 1), heights = c(1))
...
p2 + 
  geom_label(data = tibble(method = "ADEPT", total = 10),
             aes(x = total, y = 4000, label = "85% of participants have at least\n3 minutes of ADEPT-identified walking"), color = "#F56600", size = 5, inherit.aes = FALSE)
...
p2 + 
  ...
  geom_label(data = tibble(method = "stepcount", total = 30),
             aes(x = total, y = 1000, label = "98% of participants have at least\n30 minutes of stepcount-identified walking"), color = "#F56600", size = 5, inherit.aes = FALSE) 
...
::: {.incremental}
+ Sample size
+ Choice of walking identification algorithm 
+ Amount of training data
+ Paradigm for train/test partition 
+ Model choice 
...
lab_df =
  ...
  filter(metric %in% c("rank1", "rank1pct")) %>% 
  ungroup() %>%
  filter(n == 5000) %>%
  mutate(metric_lab =
...
p = all_logistic %>%
  ...
  scale_color_manual(values = c("#F56600", "#CBA052"),
                     labels = c("Rank 1", "Rank 1%"), name = "Metric")
...
ad_100 %>% 
  ...
  theme(legend.position = "none") + 
  labs(x = "Amount of data", y = "Rank 1 Accuracy (%)", title = "3 min vs. 30 min", subtitle = "ADEPT, subgroups of n = 100, random train/test") 
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Logistic</td>
      <td style="color: #F56600;">9.7</td> 
      <td>68</td>
    </tr>
    <tr>
      <td>Oversampled at 10%</td>
      <td><strong style="color: #008767;">41</strong></td>
      <td>68</td>
    </tr>
    <tr>
      <td>Weighted</td>
      <td>34</td>
      <td><strong style="color: #008767;">96</strong></td>
    </tr>
    <tr>
      <td>Two-stage</td>
      <td>20</td>
  ...
  <caption>
    Rank 1 and rank 1% accuracies of different model types on the entire population (n=13367)
  </caption>
</table>
...
# write_rds(plot_list, here::here("data/well_pred.rds"))
...
# write_rds(plot_list2, here::here("data/poor_pred.rds"))
